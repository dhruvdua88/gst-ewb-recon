...
export const reconcile = (
  ...
  // Buyers who DO have at least one matched (supplier-generated) e-way bill. If an
  // unmatched invoice's buyer never appears here, the e-way bill was most likely raised
  // by the recipient / transporter (ex-works / buyer-arranged transport) and so is absent
  // from the supplier's own EWB export — not a genuine non-generation.
  const matchedBuyers = new Set<string>();
  [...completely_matched, ...variances].forEach((r) => { if (usableGstin(r.buyer_gstin)) matchedBuyers.add(r.buyer_gstin); });
  ...
  gstrMap.forEach((g) => {
    ...
    else if (usableGstin(g.buyer_gstin) && !matchedBuyers.has(g.buyer_gstin))
      reason = 'Buyer has no e-way bills in the data — EWB likely generated by recipient/transporter (ex-works); verify';
